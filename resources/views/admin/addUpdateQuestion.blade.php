...
@section('content')
   <div class="question">
   	<form action="{{ route('admin.addUpdateQuestion') }}" method="POST">
   		@csrf
   		<div class="form-group">
   			<label for="question">
   				Question
   			</label>
   			<textarea class="form-control" name="question" cols="30" rows="3" id="question" value="{{ $result->question }}" required>{{ $result->question }}</textarea>
            <input type="number" name="id" name="id" value="{{ $result->id }}" hidden>
   		</div>
   		<div class="form-group">
   		...
   		<div class="form-group">
   			...
   			<input class="form-control" type="text" name="answer" value="{{ $result->ans }}" required>
   		</div>
   		<div class="form-group">
